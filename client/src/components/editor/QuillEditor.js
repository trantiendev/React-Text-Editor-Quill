        ...
        axios.get(link)
          .then(payload => {
            ...
            for (let node of this.getMetaTagElements(payload)) {
              if (node.getAttribute("property") === "og:title") title = node.getAttribute("content")                
              
              if (node.getAttribute("property") === "og:image") image = node.getAttribute("content")
                
              if (node.getAttribute("property") === "og:url") url = node.getAttribute("content")
                
              // if (node.getAttribute("property") === "og:description") description = node.getAttribute("content")
            }
...
class QuillEditor extends React.Component {
  ...
  constructor(props) {
    ...
    // When you press I V F P go to insertImage: this.imageHandler and click on inputOpenImageRef there.
    imageHandler = () => {
        ...
        axios.post('/api/blog/uploadfiles', formData, config)
            .then(response => {
              if (response.data.success) {
                  ...
                  // First, put an image in the node server, and then put it in src under here,
                  // Go to the image blot and create the image, take the src and alt from the ballot and put it in the editorHTML.
  ...
  modules = {
    ...
    toolbar: {
      container: "#toolbar",
      //id ="toorbar" is where B I U S I V F P is on it.
      handlers: {
